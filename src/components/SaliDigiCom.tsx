...
export default function SaliDigiCom({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setIsSubmitted(true);
    const subject = encodeURIComponent(`Nouveau Projet SALI DigiCom - de ${formData.company}`);
    ...
    window.location.href = `mailto:contact@sali-digicom.com?subject=${subject}&body=${body}`;
  };
        ...
        <section id="digicom-contact" className="relative h-[100dvh] bg-[#ebf1f8] overflow-hidden text-[#1c2c46]">
          ...
          <div className={`relative z-10 w-full h-full overflow-y-auto pt-24 pb-20 lg:py-12 px-[5vw] ${sectionPadds}`}>
            <div className="relative z-10 flex flex-col justify-center items-center w-full max-w-7xl mx-auto min-h-full lg:h-full px-4 pt-12 lg:pt-0">
              <div className="relative z-10 flex flex-col lg:grid lg:grid-cols-2 min-h-full w-full border border-[#d3dfed] rounded-3xl overflow-hidden shadow-2xl bg-white">
                    ...
                    {!isSubmitted ? (
                      <form onSubmit={handleSubmit} className="space-y-4 w-full">
                        ...
                        <button 
                          ...
                          data-d="6"
                        >
                          {t.contact.btnSendDigicom} <Send size={14} strokeWidth={3} />
                        </button>
                      </form>
                    ) : (
                      <motion.div 
